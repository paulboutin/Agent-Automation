...
for issue in "${issue_numbers[@]}"; do
  ...
  start_args+=("${issue}")
  info_json="$(./{{AUTOMATION_ROOT}}/hooks/local-worker-start.sh "${start_args[@]}")"
  ...
  if [[ "${run_agent}" == "true" ]]; then
    ...
    window_cmd="bash -lc 'cd \"${worktree}\" && ./{{AUTOMATION_ROOT}}/hooks/local-worker-run-and-route.sh --issue \"${issue}\" --branch \"${branch}\" --prompt \"${prompt}\" --host \"${host}\" --cost-profile \"${cost_profile}\" --reasoning-effort \"${reasoning_effort}\" --model \"${worker_model}\" --variant \"${variant}\" ${auto_finish_arg}; echo; echo \"Session complete\"; exec bash -l'"
  else
    window_cmd="bash -lc 'cd \"${worktree}\" && echo \"Host: ${host}\" && echo \"Prompt: ${prompt}\" && echo \"Run: ./{{AUTOMATION_ROOT}}/hooks/local-worker-run-and-route.sh --issue ${issue} --branch ${branch} --prompt ${prompt} --host ${host} --cost-profile ${cost_profile} --reasoning-effort ${reasoning_effort} --model ${worker_model} --variant ${variant}\"; exec bash -l'"
  fi
