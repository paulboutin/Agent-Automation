...
launch_plan="$("./{{AUTOMATION_ROOT}}/hooks/merge-daemon-launch-next.sh" --json 2>/dev/null || echo '{"selected":[],"candidates":[],"occupiedLanes":[]}')"

ready_issues="$(jq \
  '[.[] | select(any(.labels[]?; .name == "ready"))] | length' \
  <<< "${open_issues}")"
blocked_issues="$(jq \
  '[.[] | select(any(.labels[]?; .name == "{{NEEDS_DECISION_LABEL}}"))] | length' \
  <<< "${open_issues}")"
launch_candidates="$(jq '.selected | length' <<< "${launch_plan}")"
active_pr_count="$(jq \
  '[.[] | select(.headRefName | startswith("{{WORKER_BRANCH_PREFIX}}"))] | length' \
  <<< "${open_prs}")"
...
pr_dependency_rows="$(jq -r '
  ...
  [
    .[]
    | select(.headRefName | startswith("{{WORKER_BRANCH_PREFIX}}"))
    | . as $pr
    | dependency_tokens[]
...
stale_pr_rows="$(jq -r \
  ...
  '[.[] | select(.headRefName | startswith("{{WORKER_BRANCH_PREFIX}}")) | . as $pr
...
jq '[.[] | select(.headRefName | startswith("{{WORKER_BRANCH_PREFIX}}"))]' <<< "${open_prs}"
echo
echo "=== Blocker issues ==="
jq '[.[] | select(any(.labels[]?; .name == "{{NEEDS_DECISION_LABEL}}"))]' <<< "${open_issues}"
echo
